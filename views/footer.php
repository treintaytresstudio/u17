<!-- new post -->
<?php include 'new_post.php' ?>

<!-- cta post -->
<?php include 'cta_post.php' ?>

<!-- link no valid -->
<?php include 'link_no_valid.php' ?>

</body>
    <script src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
	<script src="https://www.gstatic.com/firebasejs/4.1.3/firebase.js"></script>
	<script src="app/js/firebase-config.js"></script>
	<script src="public/js/vendor/hashtag.min.js"></script>
    <script src="public/js/vendor/jquery.easy-autocomplete.js"></script>
	<script src="app/js/auth.js"></script>
	<script src="app/js/ready.js"></script>
    <script src="app/js/app.js"></script>
    <script src="app/js/profile.js"></script>
    <script src="app/js/settings.js"></script>
    <script src="app/js/search.js"></script>
    <script src="app/js/post.js"></script>
    <script src="app/js/comments.js"></script>
    <script src="app/js/hashtag.js"></script>
    <script src="app/js/notifications.js"></script>
    


</html>